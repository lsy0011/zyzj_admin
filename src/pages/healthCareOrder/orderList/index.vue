<template>
    <div class='orderList'>
        ...
        <div class="i-layout-page-header">
            <PageHeader title="医药保健订单管理" content="订单列表" hidden-breadcrumb />
        </div>
        <div class="orderlist_main">
            <div class="search_top" style="margin-bottom: 20px;">
        ...
        <Modal @on-visible-change="visibleChange" v-model="modal" footer-hide width="70%" title="订单详情">
            ...
            <div class="detail_box">
                ...
                <div class="orderinfi_box_l">
                    ...
                    <Row style=" margin: 15px 0px ;">
                        ...
                        <Col span="7"><span>{{ list.buyerInfo.nickName }}</span></Col>
                    ...
                    <Row style=" margin: 15px 0px ;">
                        ...
                        <Col span="7"><span>{{ list.merchant.name }}</span></Col>
                        <Col span="5"><strong>收货人手机号:</strong></Col>
                        <Col span="7"><span> {{ list.addr && list.addr.addr ? list.addr.addr.linkman : '---' }} </span>
                        </Col>
                    </Row>
                    <Row style=" margin: 15px 0px ;">
                        <Col span="5"><strong>发货地址:</strong></Col>
                        <Col span="7"><span>--上海wework--</span>
                        </Col>
                    ...
                    <Row style=" margin: 15px 0px ;">
                        ...
                        <Col span="7"><span>{{ list.status.name }}</span></Col>
                        <!-- <Col span="5"><strong>商品分类:</strong></Col>
                ...
                <div class="orderinfi_box_r">
                    <div class="orderinfi_up">
                        ...
                        <div class="order_type_receiving" v-if="list.status.code == 'wait_receiving'"> 平台已发货 </div>
                        <div class="order_type_cancel"
                            v-else-if="list.status.code == 'cancel' || list.status.code == 'expired'">
                            交易已终止
                        </div>
                        <div class="order_type_success" v-else-if="list.status.code == 'success'">
                            交易已完成
                        </div>
                        <div class="order_type_deliver" v-else>平台交易中</div>
                    </div>
                    <div class="orderinfi_down">
                        <p><strong>订单物流状态：</strong></p>
                        <div class="down_u"
                            v-if="list.status.code == 'wait_confirm' || list.status.code == 'wait_receiving' || list.status.code == 'success'">
                            <div><strong>快递公司：</strong>
                        ...
                        <div style="text-align: center;" v-else>
                            <strong style="color: #ff0000;font-size: 60px;">{{ list.status ? list.status.name : ''
                            }}</strong>
                        </div>
                    </div>
                </div>
            ...
            <div class="goods_box">
                <Table :columns="orderDetail" :data="item" border class="top20" size="small">
                    ...
                    <template #odd="{ row }">
                        <div
                            v-if="row.status.code == 'expired' || row.status.code == 'wait_pay' || row.status.code == 'cancel'">
                            未支付
                        </div>
                        <div v-else> --微信-- </div>
                    </template>
        ...
        <!-- 发货 -->
        <Modal v-model="sendFlag" title="填写运单号" width="330">
            <div>
            ...
            <template #footer>
                ...
                <Button @click="sendFlag = false; sendGoodCode = ''">取消</Button>
            </template>
        </Modal>
    </div>
</template>
        ...
        return {
            ...
            columns: [
                ...
                // },
                {
                    ...
                    align: "center",
                    render: (h, params) => {
                        return h('div', params.row.payInfo && params.row.payInfo.channel ? params.row.payInfo.channel.name || '--' : '未支付')
                    }
                },
                {
                    title: '用户昵称',
                    ...
                    align: "center",
                    render: (h, params) => {
                        return h('div', params.row.buyerInfo ? params.row.buyerInfo.realName ? params.row.buyerInfo.realName : params.row.buyerInfo.nickName : '---')
                    }
                },
                {
                    title: '用户类型',
    ...
    methods: {
        ...
        getGoodStatus() {
            ...
            getGoodsCategoryTreeApi({ biz: 'medical' }).then(res => {
                this.categoryTree = jsonToArray(res, true);
            })
        },
        // 状态 区分
        selectState(state) {
        ...
        edit(row) {
            ...
            getLogisticsTrackingListApi({ bizId: row.id }).then(res => {
                this.logisticsList = JSON.parse(JSON.stringify(res));
            })
        },
        // 物流信息
        getSummaryDetail() {
        ...
        exportOrderFn() {
            ...
            getGoodsOrderExportApi({ ...this.search, biz: 'medical' }).then(res => {
