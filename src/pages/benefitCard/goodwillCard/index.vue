<template>
    <div class='goodwillCard'>
        <!-- <h5>善益卡管理</h5> -->
        <div class="i-layout-page-header">
            <PageHeader title="权益卡管理" content="善意卡管理" hidden-breadcrumb />
        </div>
        <div class="goodwill_main">
            <div class="search_top">
                <Space size="large" wrap>
                    <Space>
                        <strong>发放时间</strong>
                        <DatePicker type="date" size="small" placeholder="开始时间" style="width: 160px" />
                        <strong>至</strong>
                        <DatePicker type="date" size="small" placeholder="截至时间" style="width: 160px" />
                    </Space>
                    <Space>
                        <strong>状态</strong>
            ...
            <div class="card_title">
                <Space size="large">
                    <strong style="font-size: 16px;line-height: 32px;">
                        <Icon type="md-list" />善益卡列表
                    </strong>
                    <Button type="primary" @click=" editCardFn(false, 'list')" size="small">新建</Button>
                </Space>
            </div>
        ...
        <Modal class-name="vertical-center-modal" v-model="modal" @on-visible-change="visibleChange" title="创建新卡">
            <Form ref="list" :model="list" :rules="addValidate" :label-width="120">
                ...
                <!-- <FormItem label="兑换积分：">
                    <Input v-model="list.exchangeCredits" placeholder="请输入"  ></Input>
                </FormItem> -->
                <!-- <FormItem label="库存：">
                    <Input v-model="list.name" placeholder="请输入"  ></Input>
                </FormItem> -->
                <!-- <template v-if="list.mode != 'manu'"> -->
        ...
        <Modal title="手动发放" v-model="modelTag" width="500">
            <Form ref="tagList" :model="tagList" :rules="tagValidate" :label-width="100">
                ...
                <FormItem label="用户类型：">
                    ...
                    <RadioGroup v-model="userType" placeholder="请先选择用户类型">
                        <Radio label="Disabled">
                            <span>残疾人 </span>
                        </Radio>
                        <Radio label="DisabledFamily">
                            <span>残疾人家属 </span>
                        </Radio>
                    </RadioGroup>
                    <span v-color="'#ccc'">（请先选择用户类型）</span>
                </FormItem>
                <FormItem label="选择用户：" prop="userId">
                    <Select v-model="tagList.userId" filterable multiple placeholder="输入用户手机号查找"
                        :remote-method="remoteMethod1" :loading="loading1" :disabled="!userType">
    ...
    data() {
        return {
            ...
            columns: [
                ...
                //     width: '150',
                //     render: (h, params) => {
                //         return h('div', [params.row.stage ? params.row.stage.level : '---'])
                //     }
                // },
                {
                    title: '发放库存',
            ...
            tagValidate: {
                ...
                userId: [
                    { required: true, type: 'array', min: 1, message: '请输入发行用户', trigger: 'change' }
                ],
                effectTime: [
                    { required: true, type: 'date', message: '请选择发放时间', trigger: 'change' }
                ],
        ...
        editCardFn(row, name) {
            if (row) {
                ...
                this.list = {
                    ...
                    exchangeCredits: data.exchangeCredits,
