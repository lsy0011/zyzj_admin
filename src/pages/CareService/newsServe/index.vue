<template>
    <div class='newsServe'>
        ...
        <div class="i-layout-page-header">
            <PageHeader title="关爱服务管理" content="新增服务管理" hidden-breadcrumb />
        </div>
        <div class="newsOrgan_main">
            <div class="top_title">新增服务管理</div>
            <div class="new_body">
...
export default {
    ...
    created() {
        this.serveId = this.$route.query.data

        if (this.serveId) {
            this.loadingSke = true;
            getServiceInfoDetailApi(this.serveId)
                .then(res => {
                    let url = [];
                    if (res.file) {
                    ...
                    this.list = {
                        ...
                        cyCardDiscount: res.cyCardDiscount,
                        nonprofit: res.nonprofit,
                        donateType: res.donateType ? res.donateType.code : '',
    ...
    methods: {
        ...
        // 获取服务分类列表
        getCategory(id = '') {
            getServiceCategoryListApi({ biz: 'common', merchantId: id })
                .then(res => {
                    res.forEach(element => {
