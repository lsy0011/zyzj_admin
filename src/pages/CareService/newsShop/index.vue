<template>
    <div class='newsShop'>
        ...
        <div class="i-layout-page-header">
            <PageHeader title="关爱服务管理" :content="titleName" hidden-breadcrumb />
        </div>
        <div class="newsShop_main">
            <h1 class="top_title">{{ titleName }}</h1>
...
export default {
    ...
    methods: {
            ...
            getStoreInfoDetailApi(this.doorId)
                .then(res => {
                    ...
                    this.list = {
                        ...
                        categoryId: res.region ? res.category.id : '',
                        name: res.name,
                        address: res.address,
                        principal: res.principal,
                        tel: res.tel,
                        tags: res.tags,
                        opType: res.opType.code,
        ...
        saveShop() {
            ...
            formData.append('workWeekDay', this.workWeekDay ? this.workWeekDay : '');   // 营业日期
            formData.append('coordinates', this.mapLocation.join());   // 坐标
        ...
        // 获取 分类 列表
        getCateoryList() {
            getSupplierCategoryTreeApi({ biz: 'common' })
                .then(res => {
                    res.forEach(element => {
