<template>
    <div class='serveManage'>
        ...
        <div class="i-layout-page-header">
            <PageHeader title="关爱服务管理" content="服务管理" hidden-breadcrumb />
        </div>
        <div class="serveManage_main">
            ...
            <Table border :loading="loading" :columns="columns" :data="data">
                ...
                <template #action="{ row }">
                    <Button type="primary" size="small" style="margin-right: 10px;" @click=" jumpNew(row)">编辑</Button>
                    <Poptip confirm transfer title="确认删除该服务吗?" @on-ok=" removeServiceFn(row)">
...
import {
    ...
    postServiceInfoRemoveApi, // 服务 删除
...
export default {
    ...
    methods: {
        ...
        remoteMethodStore(query) {
            if (query !== '') {
                ...
                this.storeTime = setTimeout(() => {
                    getStoreInfoPageApi({ biz: 'common', keywords: query, pageSize: 200 }).then(res => {
        ...
        //  获取服务分类列表
        getServeCate() {
            getServiceCategoryListApi({ biz: 'common' })
                .then(res => {
                    if (res.length > 0) {
                        res.forEach(element => {
