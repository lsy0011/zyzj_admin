<template>
    <div class='discountsPayment'>
        ...
        <div class="i-layout-page-header">
            <PageHeader title="医疗服务管理" content="优惠付订单管理" hidden-breadcrumb />
        </div>
            ...
            <div class="search_top">
                <Space size="large" wrap>
                    ...
                    <Space>
                        ...
                        <!-- 支持模糊查询 -->
                        <Button type="primary" size="small" @click="pageNumber = 1; getOrderList()">查询</Button>
        ...
        return {
            columns: [
                ...
                },
                {
                    ...
                    width: 180,
                    render: (h, params) => {
                        return h('div', params.row.payInfo && params.row.payInfo.channel ? params.row.payInfo.channel.name : '---')
                    }
                },
                {
                    title: '用户昵称',
                    ...
                    width: 180,
                    render: (h, params) => {
                        return h('div', params.row.buyerInfo ? (params.row.buyerInfo.name ? params.row.buyerInfo.name : params.row.buyerInfo.nickName ? params.row.buyerInfo.nickName : params.row.buyerInfo.realName ? params.row.buyerInfo.realName : params.row.buyerInfo.mobile) : '---')
                    }
                },
                {
                    title: '用户手机号',
