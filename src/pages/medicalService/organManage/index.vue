<template>
    <div class='organManage'>
        ...
        <div class="i-layout-page-header">
            <PageHeader title="医疗服务管理" content="机构管理" hidden-breadcrumb />
        </div>
        <div class="shopManage_main">
                ...
                <Space size="large">
                    ...
                    <Space>
                        <Input v-model="search.keywords" placeholder="请输入手机号/名称等关键字" clearable size="small"
                            style="width: 200px;" />
                        <Button type="primary" size="small" @click="pageNumber = 1; getBaseList()">
                            查询</Button>
            ...
            <div class="modul_add" style="margin-bottom: 15px;">
                <Space size="large">
                    <strong style="font-size: 17px; line-height: 32px;">
                        <Icon type="md-reorder" />机构列表
                    </strong>
                    <Button type="primary" @click=" this.$router.push('newsOrgan')">添加</Button>
                </Space>
            </div>
            <Table border :columns="columns" :data="data" :loading="loading">
                ...
                <template #action="{ row }">
                    <Button v-show="row.fzChannel && row.fzChannel.code == 'unknown'" type="info" size="small" v-margin="5"
                        @click="jumpSubInfo(row)">分账信息</Button>
                    <Button type="primary" size="small" @click=" editAndNewFn(row)" v-margin="5">编辑</Button>
                    ...
                    <Button type="info" size="small" @click="jumpShop(row)">管理门店</Button>
                </template>
            </Table>
